function teleporter:platform/check
...
# remove
execute as @e[tag=TpPlatform,scores={AT.isIntact=..8}] at @s run function teleporter:platform/remove

schedule function teleporter:platform/destroy 1s