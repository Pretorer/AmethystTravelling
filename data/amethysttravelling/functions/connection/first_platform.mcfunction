...
execute as @a[predicate=amethysttravelling:has_magical_item,scores={AT.isConnecting=0,AT.blocked=0}] at @s if entity @e[tag=TpPlatform,scores={AT.isConnecting=0,AT.isConnected=0},distance=..1.5] run function amethysttravelling:connection/first_connect
